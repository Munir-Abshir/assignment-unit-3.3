...
for (let numb of supplyChanges) {
  if (numb > 0) {
    stretchPositives.push(numb);
  } else if (numb < 0) {
    stretchNegatives.push(numb);
  } else {
    stretchZeroes.push(numb);
  }
...
let totalParts = 0;
for (let hskowns of supplyChanges) {
totalParts = totalParts + hskowns;
}
